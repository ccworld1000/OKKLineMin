...
/** Represents the results of executing a query on an `<CCSQLite>`.
...
@interface CCResultSet : NSObject
...
/** `CCStatement` used by result set. */

@property (atomic, retain) CCStatement *statement;

///------------------------------------
/// @name Creating and closing database
///------------------------------------

/** Create result set from `<CCStatement>`
 
 @param statement A `<CCStatement>` to be performed
 
 @param aDB A `<CCSQLite>` to be used
 
 @return A `CCResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(CCStatement *)statement usingParentDatabase:(CCSQLite*)aDB;

/** Close result set */

- (void)close;

- (void)setParentDB:(CCSQLite *)newDb;
