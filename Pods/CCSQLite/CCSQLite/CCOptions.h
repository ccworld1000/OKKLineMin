...
typedef NS_ENUM(NSInteger, CCOptionsCorruptAction) {
    CCOptionsCorruptAction_Fail   = 0,
    CCOptionsCorruptAction_Rename = 1,
    CCOptionsCorruptAction_Delete = 2,
};

typedef NS_ENUM(NSInteger, CCOptionsPragmaSynchronous) {
    CCOptionsPragmaSynchronous_Off    = 0,
    CCOptionsPragmaSynchronous_Normal = 1,
    CCOptionsPragmaSynchronous_Full   = 2,
};

#ifdef SQLITE_HAS_CODEC
typedef NSData *_Nonnull (^CCSQLiteCipherKeyBlock)(void);
#endif

@interface CCOptions : NSObject <NSCopying>

/**
 * How should YapDatabase proceed if it is unable to open an existing database file
 * because sqlite finds it to be corrupt?
 *
 * - CCOptionsCorruptAction_Fail
 *     The YapDatabase alloc/init operation will fail, and the init method will ultimately return nil.
 *
 * - CCOptionsCorruptAction_Rename
 ...
 *
 * - CCOptionsCorruptAction_Delete
 *     The YapDatabase init operation will succeed, a new database file will be created,
 *     and the corrupt file will be deleted.
 *
 * The default value is CCOptionsCorruptAction_Rename.
 **/
@property (nonatomic, assign, readwrite) CCOptionsCorruptAction corruptAction;
 ...
 *
 * The default value is CCOptionsPragmaSynchronous_Full.
 **/
@property (nonatomic, assign, readwrite) CCOptionsPragmaSynchronous pragmaSynchronous;
 ...
 * - initialize YapDatabase with corresponding CCOptions
 ...
 **/
@property (nonatomic, copy, readwrite) CCSQLiteCipherKeyBlock cipherKeyBlock;
