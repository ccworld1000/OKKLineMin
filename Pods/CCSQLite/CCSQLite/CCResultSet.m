...
+ (instancetype) resultSetWithStatement:(CCStatement *)statement usingParentDatabase:(CCSQLite *)aDB {
    ...
    return CCReturnAutoreleased(rs);
}
...
- (void) dealloc {
    ...
    CCRelease(_columnNameToIndexMap);
    _columnNameToIndexMap = nil;
}

- (void) close {
    [_statement reset];
    CCRelease(_statement);
    _statement = nil;
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"CCSQLite" code:SQLITE_MISUSE userInfo:errorMessage];
            }
